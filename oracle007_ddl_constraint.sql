...
무결성 제약조건
    ...
    제약조건은 바람직하지않는 데이터가 저장되는 것을 방지하는 것을 말한다.
...
무결성 제약조건 5종류 : not null, unique, primary key, foreign key, check ★★★★★
    ...
    5. check : 저장가능한 데이터의 값의 범위나 조건을 지정하여 설정한 값만을 허용한다.
...
무결성 제약조건 2가지 레벨 : 컬럼레벨, 테이블 레벨
    ...
    CREATE TABLE emp06(
     ...
     CREATE TABLE emp07(
        ...
        constraint emp07_ck check(age between 20 and 50),
        constraint emp07_gen_ck check(gen in('m','w'))
     );
     
==============================================================================*/

SELECT * FROM user_constraints; -- 데이터 딕셔너리에 저장되어있음
...
-- deptno에는 값이10, 20, 30만 있음
INSERT INTO emp08(empno, deptno, locno)
VALUES(4, 40, 11);

-- ORA-02291: integrity constraint (HR.EMP08_LOCNO_FK) violated - parent key not found
-- locno에는 값이 11,12,13만 있음
INSERT INTO emp08(empno, deptno, locno)
VALUES(5, 30, 14);

-- emp08테이블의 deptno컬럼에서 30을 참조하지 않기 때문에 이상없이 삭제가능하다.
DELETE FROM dept01
WHERE deptno = 30;
...
/*
다른 테이블에 현재 테이블을 참조해서 사용하고 있을 때는 제약조건을 제거한 후 현재 테이블의 데이터를 삭제한다.
*/

SELECT * FROM user_constraints
WHERE constraint_name LIKE '%EMP08%';

-- deptno = 20과 연결되어있는 컬럼 연결 끊기
ALTER TABLE emp08
